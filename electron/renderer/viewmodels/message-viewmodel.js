...
/**
 * Transform batch of messages to view models
 * Filters out non-displayable messages
 *
 * @param {Array<Object>} messages - Array of message objects
 * @returns {Array<Object>} Array of view models for displayable messages
 *
 * @example
 * const viewModels = createMessageListViewModel([
 *   {role: "user", content: "Hello"},
 *   {role: "system", content: "internal"},  // filtered out
 *   {role: "assistant", content: "Hi!"}
 * ]);
 * // => [{role: "user", ...}, {role: "assistant", ...}]
 */
export function createMessageListViewModel(messages) {
  if (!Array.isArray(messages)) {
    return [];
  }

  return messages.filter(shouldDisplayMessage).map(createMessageViewModel);
}

/**
 * Calculate message statistics for UI display
 *
 * @param {Array<Object>} messages - Array of message objects
 * @returns {Object} Statistics
 * @returns {number} return.total - Total messages
 * @returns {number} return.userMessages - Count of user messages
 * @returns {number} return.assistantMessages - Count of assistant messages
 * @returns {number} return.totalCharacters - Total character count
 *
 * @example
 ...
 * ]);
 * // => {total: 2, userMessages: 1, assistantMessages: 1, totalCharacters: 18}
 */
export function calculateMessageStats(messages) {
  if (!Array.isArray(messages)) {
    return { total: 0, userMessages: 0, assistantMessages: 0, totalCharacters: 0 };
  }
