...
- `mcp_pool.py` - Connection pool for concurrent MCP server access (stdio serialization).
...
- Keep integration setup here; `api/main.py` lifespan initializes pools and passes to services.
- MCP servers are pooled (`mcp_pool.py`) to handle concurrent requests without stdio conflicts.
- Maintain clear separation between registration (registry) and runtime control (pool/handlers).
- Use structured logging with context; handle failures with explicit exceptions.

## Extending

- To add an MCP server: define it in `mcp_servers.py`, register in `mcp_registry.py`, pool will auto-manage.
